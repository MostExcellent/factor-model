...
if __name__ == "__main__":
    ...
    df['ForwardReturn'] = df.groupby('Ticker')['PX_LAST'].pct_change(periods=2)
    df['ForwardReturn'] = df.groupby('Ticker')['ForwardReturn'].shift(-2)
    df['ForwardEPS'] = df.groupby('Ticker')['IS_EPS'].shift(-2)
    # print(df.head())
    ...
    df['ForwardEPSNorm'] = df.groupby('Date')['ForwardEPS'].transform(normalize)
    ...
    with open('eps_initial_test_results.txt', 'w') as file:
        file.write(f"RMSE: {rmse}\n")
        file.write(f"R2: {r2}\n")
        file.write(f"Linear MSE: {rmse_linear}\n")
        file.write(f"Linear R2: {r2_linear}\n")
        file.write(f"Naive MSE: {rmse_naive}\n")
    ...
    plt.title('EPS Feature Importance')
    plt.savefig('eps_feature_importance.png')
    ...
    plt.savefig('eps_residuals.png')

    # Plot residuals as a function of predicted returns
    plt.figure(figsize=(10, 6))
    plt.scatter(y_pred, residuals)
    plt.title('EPS Residuals vs. Predicted Returns')
    plt.savefig('eps_residuals_vs_predicted_returns.png')

    # Correlation matrix heatmap
    plt.figure(figsize=(10, 6))
    sns.heatmap(df_grouped[features].corr(), annot=True, cmap='coolwarm')
    plt.title('EPS Correlation Matrix')
    plt.savefig('eps_correlation_matrix.png')
    ...
    plt.title('Predicted vs. Actual Earanings per Share')
    ...
    with open('eps_bootstrap_test_results.txt', 'w') as file:
        file.write(
